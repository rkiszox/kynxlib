	...
	Themes = {
		kynx = {
			["Color Hub 1"] = ColorSequence.new({
				...
				ColorSequenceKeypoint.new(0.50, Color3.fromRGB(25, 25, 28)),
				ColorSequenceKeypoint.new(1.00, Color3.fromRGB(20, 20, 22))
			}),
			["Color Hub 2"] = Color3.fromRGB(25, 25, 28),
			["Color Stroke"] = Color3.fromRGB(60, 60, 70),
			["Color Theme"] = Color3.fromRGB(255, 255, 255),
			["Color Text"] = Color3.fromRGB(235, 235, 240),
			["Color Dark Text"] = Color3.fromRGB(160, 160, 175)
		}
	},
	Info = {Version = "1.0.0"},
	Save = {UISize = {550, 380}, TabSize = 160, Theme = "kynx"},
	Settings = {},
	Connection = {},
	Instances = {},
	Elements = {},
...
local function ButtonFrame(Instance, Title, Description, HolderSize, Icon, Locked)
	...
	LabelHolder = Create("Frame", Frame, {
		...
		Create("UIPadding", {PaddingBottom = UDim.new(0, 7), PaddingTop = UDim.new(0, 7)}),
		TitleL,
		DescL,
	})
	local Label = {}
...
function kynx:MakeWindow(Configs)
	...
	local WLocked = Configs.Locked or false
	local WDraggable = Configs.Draggable ~= nil and Configs.Draggable or true
	local WResizable = Configs.Resizable ~= nil and Configs.Resizable or true
	
	WIcon = kynx:GetIcon(WIcon)
	if WIcon and WIcon ~= "" and not WIcon:find("rbxassetid://") then
		WIcon = false
	end
	if WBackground and WBackground ~= "" and not WBackground:find("rbxassetid://") then
		WBackground = false
	end
	...
	if WLocked then
		...
		local LockOverlay = Create("Frame", MainFrame, {
	...
	if WDraggable and not WLocked then MakeDrag(MainFrame) end
	...
	local TitleIcon
	local IconOffset = 0
	if WIcon and WIcon:find("rbxassetid://") then
		TitleIcon = InsertTheme(Create("ImageLabel", TitleHolder, {
			Size = UDim2.new(0, 16, 0, 16), Position = UDim2.new(0, 0, 0.5),
	...
	local Title = InsertTheme(Create("TextLabel", TitleHolder, {
		Position = UDim2.new(IconOffset, 0, 0, 2), AnchorPoint = Vector2.new(0, 0), AutomaticSize = "XY",
	...
	local SubTitle = InsertTheme(Create("TextLabel", TitleHolder, {
		Position = UDim2.new(IconOffset, 0, 0, 20), AnchorPoint = Vector2.new(0, 0), AutomaticSize = "XY",
	...
	if WLocked then
		local LockOverlay = Create("Frame", Containers, {
			Size = UDim2.new(1, 0, 1, 0),
			BackgroundTransparency = 0.3,
	...
	if WResizable and not WLocked then
		...
		ControlSize2 = Create("ImageButton", MainFrame, {
			Size = UDim2.new(0, 0, 0, 0),
			Visible = false
		})
	end
	...
	function Window:MinimizeBtn()
		...
		if Minimized then
			...
			if ControlSize1 then ControlSize1.Visible = true end
			if ControlSize2 then ControlSize2.Visible = true end
			Minimized = false
		else
	...
	function Window:MakeTab(paste, Configs)
		...
		local LabelTitle = InsertTheme(Create("TextLabel", TabSelect, {
			...
			TextColor3 = Theme["Color Text"], TextSize = 10,
			TextXAlignment = Enum.TextXAlignment.Left, TextTransparency = (FirstTab and 0.3) or 0,
			TextTruncate = "AtEnd"
		}), "Text")
		local LabelIcon
		...
		if TLocked then
			local LockOverlay = Create("Frame", Container, {
				Size = UDim2.new(1, 0, 1, 0),
				BackgroundTransparency = 0.3,
				BackgroundColor3 = Color3.fromRGB(0, 0, 0),
				ZIndex = 5
		...
		local function Tabs()
			...
			if LabelIcon then CreateTween({LabelIcon, "ImageTransparency", 0, 0.35}) end
		...
		function Tab:Disable()
			...
			if LabelIcon then CreateTween({LabelIcon, "ImageTransparency", 0.3, 0.35}) end
		...
		function Tab:AddSection(Configs)
			...
			local SectionLocked = Configs.Locked or false
			SectionIcon = kynx:GetIcon(SectionIcon)
			if SectionIcon and SectionIcon ~= "" and not SectionIcon:find("rbxassetid://") then
				SectionIcon = false
			end
			...
			if SectionLocked then
				...
				local LockIcon = Create("ImageLabel", SectionFrame, {
					Size = UDim2.new(0, 12, 0, 12),
					Position = UDim2.new(1, -8, 0.5),
					AnchorPoint = Vector2.new(1, 0.5),
					BackgroundTransparency = 1,
			...
			local SectionLabel = InsertTheme(Create("TextLabel", SectionFrame, {
				...
				Size = UDim2.new(1, SectionIcon and -25 or -15, 1),
				Position = UDim2.new(0, SectionIcon and 25 or 5),
				BackgroundTransparency = 1, TextTruncate = "AtEnd", TextSize = 13,
				TextXAlignment = "Left"
			}), "Text")
			local Section = {}
		...
		function Tab:AddParagraph(Configs)
			...
			local PLocked = Configs.Locked or false
			local PBackground = Configs.BackgroundImage or ""
			PIcon = kynx:GetIcon(PIcon)
			if PIcon and PIcon ~= "" and not PIcon:find("rbxassetid://") then
				PIcon = false
			end
			if PBackground and PBackground ~= "" and not PBackground:find("rbxassetid://") then
				PBackground = false
			end
			...
			if PIcon and PIcon:find("rbxassetid://") and not PLocked then
				local IconImage = Create("ImageLabel", Frame, {
					Size = UDim2.new(0, 16, 0, 16),
					Position = UDim2.new(1, -12, 0.5),
					AnchorPoint = Vector2.new(1, 0.5),
			...
			function LabelFunc:SetDesc(NewDesc)
				if type(NewDesc) == "string" and NewDesc:gsub(" ", ""):len() > 0 then
					DescL.Visible = true
					DescL.Text = NewDesc
					LabelHolder.Position = UDim2.new(0, 10, 0)
		...
		function Tab:AddButton(Configs)
			...
			local BLocked = Configs.Locked or false
			BIcon = kynx:GetIcon(BIcon)
			if BIcon and BIcon ~= "" and not BIcon:find("rbxassetid://") then
				BIcon = false
			end
			local Callback = Funcs:GetCallback(Configs, 2)
			local FButton, LabelFunc = ButtonFrame(Container, BName, BDescription, UDim2.new(1, -20), BIcon, BLocked)
			if BLocked then
				FButton.BackgroundTransparency = 0.5
				FButton.BackgroundColor3 = Color3.fromRGB(40, 40, 45)
			else
		...
		function Tab:AddToggle(Configs)
			...
			local Button, LabelFunc = ButtonFrame(Container, TName, TDesc, UDim2.new(1, -42), false, TLocked)
			if TLocked then
			...
			local function SetToggle(Val)
				if WaitClick or TLocked then return end
				...
				if Default then
					...
					CreateTween({Toggle, "BackgroundColor3", Color3.fromRGB(80, 80, 90), 0.25})
		...
		function Tab:AddDropdown(Configs)
			...
			local Button, LabelFunc = ButtonFrame(Container, DName, DDesc, UDim2.new(1, -180), false, DLocked)
			if DLocked then
				Button.BackgroundTransparency = 0.5
				Button.BackgroundColor3 = Color3.fromRGB(40, 40, 45)
			end
			...
			local DropFrame = Create("Frame", NoClickFrame, {
				Size = UDim2.new(SelectedFrame.Size.X, 0, 0), BackgroundTransparency = 0.05,
				BackgroundColor3 = Color3.fromRGB(30, 30, 35), AnchorPoint = Vector2.new(0, 1),
			...
			local ScrollFrame = InsertTheme(Create("ScrollingFrame", DropFrame, {
				...
				AutomaticCanvasSize = "Y", Active = true
			}, {
			...
			local function Minimize()
				if WaitClick or DLocked then return end
				WaitClick = true
				if NoClickFrame.Visible then
					Arrow.Image = "rbxassetid://10709791523"
					CreateTween({Arrow, "ImageColor3", Color3.fromRGB(255, 255, 255), 0.2})
			...
			local AddNewOptions, GetOptions, AddOption, RemoveOption, Selected do
				...
				AddOption = function(index, Value)
					...
					local OptioneName = InsertTheme(Create("TextLabel", Button, {
						...
						Font = Enum.Font.GothamBold, TextXAlignment = "Left",
			...
			function Dropdown:Add(...)
				local NewOptions = {...}
				if type(NewOptions[1]) == "table" then table.foreach(Option, function(_,Name) AddOption(Name) end)
				else table.foreach(NewOptions, function(_,Name) AddOption(Name) end) end
			end
			function Dropdown:Remove(Option)
				for index, Value in pairs(GetOptions()) do
					if type(Option) == "number" and index == Option or Value.Name == "Option" then RemoveOption(index, Value.Value) end
				end
			end
		...
		function Tab:AddSlider(Configs)
			...
			local SLocked = Configs.Locked or false
			if CheckFlag(Flag) then Default = GetFlag(Flag) end
			Min, Max = Min / Increase, Max / Increase
			local Button, LabelFunc = ButtonFrame(Container, SName, SDesc, UDim2.new(1, -180), false, SLocked)
			if SLocked then
				Button.BackgroundTransparency = 0.5
				Button.BackgroundColor3 = Color3.fromRGB(40, 40, 45)
			end
		...
		function Tab:AddTextBox(Configs)
			...
			local Button, LabelFunc = ButtonFrame(Container, TName, TDesc, UDim2.new(1, -38), false, TLocked)
			if TLocked then
				Button.BackgroundTransparency = 0.5
				Button.BackgroundColor3 = Color3.fromRGB(40, 40, 45)
			end
		...
		function Tab:AddDiscordInvite(Configs)
			...
			local LTitle = InsertTheme(Create("TextLabel", FrameHolder, {
				...
				TextXAlignment = "Left", BackgroundTransparency = 1,
				TextSize = 10, Text = Title
			}), "Text")
			local LDesc = InsertTheme(Create("TextLabel", FrameHolder, {
				Size = UDim2.new(1, -52, 0, 0), Position = UDim2.new(0, 44, 0, 22),
				TextWrapped = "Y", AutomaticSize = "Y", Font = Enum.Font.Gotham,
